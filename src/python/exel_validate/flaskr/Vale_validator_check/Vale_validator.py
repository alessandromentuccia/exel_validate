...
class Validator_v():
    ...
    def ck_QD_description(self, df_mapping, sheet_QD):
        ...
        sheet = xfile.get_sheet_by_name("Sheet 1") #modificare sheet name

        for index, row in df_mapping.iterrows(): 
            QD_list = row["CD_QUESITO_DIAGNOSTICO"].split(",") #lista dei codici QD
            ...
            if QD_list != "":
                ...
                flag_error_QD = False
                #cell_QD_value = Validator_v.findCell_col_all(self, sheet_QD, disciplina, 0, 1) #[] 

                for QD in QD_list: #ciclo per ogni QD
                    QD = QD.strip()
                    if QD != "":
                        #elimino eventuali spazi ai bordi
                        rowQD, colQD = Validator_v.findCell_col(self, sheet_QD, QD, 5, 6)
                        ...
                        '''if rowQD == -1 or Validator_v.findCell(self, sheet_QD, QD, 5, 6) == -1:
                            print("vuoti_description_QD: " + QD)
                            QD_record.append(QD)
                            flag_error_QD = True'''


                sheet["AJ"+str(int(index)+2)] = ",".join(string_record) #modificare colonna QD descrizione
    ...
    def ck_metodiche_description(self, df_mapping, sheet_Metodiche):
        ...
        for index, row in df_mapping.iterrows(): 
            metodica_list = row["CD_METODICA"].split(",") #lista dei codici QD
            cod_siss =  row["COD_SISS"]
            ...
            if metodica_list != "":
                ...
                cell_metodica_value = Validator_v.findCell_all_SISS(self, sheet_Metodiche, cod_siss, 2, 3) #[] 

                for metodica in metodica_list: #ciclo per ogni QD
                    metodica = metodica.strip() #elimino eventuali spazi ai bordi
                    if metodica != "":
                        rowmetodica, colmetodica = Validator_v.findCell_col(self, sheet_Metodiche, metodica, 4, 5)
                        ...
                        '''if rowmetodica == -1 or Validator_v.findCell(self, sheet_Metodiche, metodica, 4, 5) == -1:
    ...
    def ck_distretti_description(self, df_mapping, sheet_Distretti):
        ...
        for index, row in df_mapping.iterrows(): 
            distretti_list = row["CD_DISTRETTO"].split(",") #lista dei codici distretti
            cod_siss =  row["COD_SISS"]

            if distretti_list != "":
                string_record = []
                prestazione_record = [] #QD che non hanno la disciplina corretta
                distretti_record = [] #QD non trovati e da riportare in error
                flag_error_prestazione = False
                flag_error_distretti = False
                cell_distretti_value = Validator_v.findCell_all_SISS(self, sheet_Distretti, cod_siss, 2, 3) #[] 

                for distretti in distretti_list: #ciclo per ogni QD
                    distretti = distretti.strip() #elimino eventuali spazi ai bordi
                    if distretti != "":
                         
                        rowdistretti, coldistretti = Validator_v.findCell_col(self, sheet_Distretti, distretti, 4, 5)

                        '''if cell_distretti_value != []:
                            if distretti not in cell_distretti_value:
                                print("distretti: " + distretti + ", cell_distretti_value: %s", cell_distretti_value)
                                prestazione_record.append(distretti)   
                                flag_error_prestazione = True'''
                        
                                
                        if rowdistretti != -1:
                            #resultQDdisciplina = sheet_QD.cell_value(rowQD, 0) #valore disciplina QD da catalogo
                            resultdistretti_description = sheet_Distretti.cell_value(rowdistretti, 5) #valore descrizione distretti da catalogo
                            string_record.append(resultdistretti_description)
                        '''if rowdistretti == -1 or Validator_v.findCell(self, sheet_Distretti, distretti, 4, 5) == -1:
                            print("distretti non_esiste: " + distretti)
                            distretti_record.append(distretti)
                            flag_error_distretti = True'''

                        
                sheet["AP"+str(int(index)+2)] = ",".join(string_record) #modificare colonna QD descrizione
                print("descrizione aggiunta")

                '''st_error = ""
                if flag_error_prestazione == True: #controllo se c'è stato errore di desciplina nella riga
                    st_error = "alert: codice prestazione SISS non corrispondente per distretti: " + ",".join(prestazione_record) + "\n"
                    print("errore relazione distretti codice SISS")
                    error_dict["distretti_non_in_prestazione"].append(str(int(index)+2))

                
                if flag_error_distretti == True:
                    st_error = st_error + "alert: distretti non presente nel catalogo: " + ",".join(distretti_record)
                    error_dict["distretti non_esiste"].append(str(int(index)+2))
