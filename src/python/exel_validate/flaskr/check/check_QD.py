...
class Check_QD(Check_action):
    ...
    def ck_QD_agenda(self, error_dict):
        ...
        for index, row in self.file.df_mapping.iterrows():
            if row[self.file.work_abilitazione_esposizione_siss] == "S":
                index_agenda_dict = self.update_list_in_dict(index_agenda_dict, row[self.file.work_codice_agenda_siss], str(int(index)+2))
                QD_agenda_dict[str(int(index)+2)] = str(row[self.file.work_codice_QD])
        ...
        for key, indice in index_agenda_dict.items(): #key: AGENDA, value: INDICE
            ...
            if len(indice) > 1:
                for ind in indice: 
                    ...
                    QD_prestazione = QD_string.split(self.file.work_delimiter) #lista dei QD
    ...
    def ck_QD_disciplina_agenda(self, error_dict):
        ...
        for index, row in self.file.df_mapping.iterrows():
            ...
            if row[self.file.work_abilitazione_esposizione_siss] == "S":
                if row[self.file.work_codice_agenda_siss] not in agende_viewed and row[self.file.work_codice_QD] is not None:
                    searchedAgenda = row[self.file.work_codice_agenda_siss]
                    
                    result = self.findCell_agenda_II(self.file.df_mapping, searchedAgenda, self.file.work_codice_agenda_siss) 
                    ...
                    if result != -1:
                        ...
                        for res in result: #per ogni risultato controllo che ci sia la stessa disciplina
                            indice = int(res)+2
                            result_QD = self.file.df_mapping[self.file.work_codice_QD].iloc[int(res)]
                            disciplina_mapping_row = self.file.df_mapping[self.file.work_codici_disciplina_catalogo].iloc[int(res)]#-1]
                            descrizione_disciplina_mapping_row = self.file.df_mapping[self.file.work_descrizione_disciplina_catalogo].iloc[int(res)]#-1]
                            #print("qui 1, index: " + str(res) + " QD: ", result_QD)
                            if result_QD != "":
                                QD_list = result_QD.split(self.file.work_delimiter)
                                for QD in QD_list:
                                    QD = QD.strip()
                                    #print("start iterate QD:" + QD)
                                    if QD != "":
                                        short_sheet = self.file.sheet_QD.loc[self.file.sheet_QD["Codice Quesito"] == QD] #ricavo uno sheet del catalogo filtrato per il QD
                                        di_list = []
    ...
    def ck_QD_sintassi(self, error_dict):
        ...
        for index, row in self.file.df_mapping.iterrows():
            ...
            if row[self.file.work_abilitazione_esposizione_siss] == "S": 
                if row[self.file.work_codice_QD] is not None:
                    row_replace = row[self.file.work_codice_QD].replace(" ", "")
                    if " " in row[self.file.work_codice_QD]:
        ...
        for ind in error_dict['error_QD_caratteri_non_consentiti']:
            ...
            if sheet[self.file.ork_alert_column+ind].value is not None:
                sheet[self.file.work_alert_column+ind] = str(sheet[self.file.work_alert_column+ind].value) + "; \n" + out_message #modificare colonna alert
            else:
                sheet[self.file.work_alert_column+ind] = out_message
    ...
    def ck_QD_descrizione(self, error_dict):
        ...
        for index, row in self.file.df_mapping.iterrows():
            if row[self.file.work_abilitazione_esposizione_siss] == "S":
                QD_list = row[self.file.work_codice_QD].split(self.file.work_delimiter)
                description_list = row[self.file.work_descrizione_QD]#.split(self.work_delimiter)
                flag_error = False
                ...
                if QD_list is not None:
                    for QD in QD_list:
                        QD = QD.strip()
                        if QD != "":
                            QD_catalogo = self.file.sheet_QD.loc[self.file.sheet_QD["Codice Quesito"] == QD]  
                            #print("QD: " + str(QD)) 
                            if QD_catalogo.empty:  
                                ...
                                if (" "+self.file.file.work_delimiter) in description_list or (self.file.work_delimiter+" ") in description_list:
                                    ...
                                    description_list = description_list.replace(self.file.work_delimiter+" ", self.file.work_delimiter) #elimino spazio dopo  del separatore
                                    description_list = description_list.replace(" "+self.file.work_delimiter, self.file.work_delimiter) #elimino spazio prima del separatore
                                ...
                                try:
                                    ...
                                    if desQD not in description_list.split(self.file.work_delimiter):
        ...
        for ind in error_dict['error_QD_descrizione']:
            ...
            if sheet[self.file.file.work_alert_column+ind].value is not None:
                sheet[self.file.work_alert_column+ind] = str(sheet[self.file.file.work_alert_column+ind].value) + "; \n" + out_message #modificare colonna alert
            else:
                sheet[self.file.work_alert_column+ind] = out_message
    ...
    def ck_QD_operatori_logici(self, error_dict):
        ...
        for index, row in self.file.df_mapping.iterrows():
            if row[self.file.work_abilitazione_esposizione_siss] == "S":
                if row[self.file.work_operatore_logico_QD] != "" and row[self.file.work_codice_QD].strip() != "":
                    agenda_SISS = row[self.file.work_codice_agenda_siss]
                    agenda_SISS_str = str(agenda_SISS).strip()
                    if agenda_SISS not in agende_checked:
                        result = self.findCell_agenda_II(self.file.df_mapping, agenda_SISS, self.file.work_codice_agenda_siss) 
                        #print("result dell'agenda '" + agenda_SISS_str + "' :", result)
                        if result != -1:
                            for res in result:
                                indice = int(res)+2
                                resultOP = self.file.df_mapping[self.file.work_operatore_logico_QD].iloc[int(res)]
                                #print("resultOP: " + resultOP + " all'indice:" + str(indice))
                                if str(row[self.file.work_operatore_logico_QD]) != str(resultOP) and str(resultOP) != "":
                                    ...
                                    print("resultOP: " + str(resultOP) + " con: " + str(row[self.file.work_operatore_logico_QD]))
                    agende_checked.append(agenda_SISS)

                elif row[self.file.work_operatore_logico_QD] == "" and row[self.file.work_codice_QD].strip() != "": 
